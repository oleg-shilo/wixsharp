namespace WixSharp
{
	...
	public class CloseApplication : WixEntity, IGenericEntity
	{
		...
		/// <summary>
		/// Property to be set if application is still running.
		/// Useful for launch conditions or to conditionalize custom UI to ask user to shut down apps.
		/// </summary>
		[Xml]
		public string Property;

		/// <summary>
		/// Name of the exectuable to be closed.
		...
		/// <summary>
		/// Optionally sends a close message to the application from deffered action without impersonation. 
		/// </summary>
		[Xml]
		public bool ElevatedCloseMessage = false;

		/// <summary>
		/// Sends WM_QUERYENDSESSION then WM_ENDSESSION messages to the application from a deffered action without impersonation.
		/// </summary>
		[Xml]
		...
		/// "Abort" will cancel the install.
		/// "Retry" will attempt the check again and if the application is still running, prompt again.
		/// "Ignore" will continue and execute any other options set on the CloseApplication element. 
		/// </summary>
		[Xml]
